...
// 初始化空白图像、点云内存，预计算常量，避免每次调用时重复计算，计算2D占据栅格尺寸
void ContourDetector::Init(const ContourDetectParams& params) {
    cd_params_ = params;    // 传感器范围、栅格分辨率、图像模糊程度等参数

    new_corners_cloud_   = PointCloudPtr(new pcl::PointCloud<PCLPoint>()); // 分配角点云内存

    // 计算占据图尺寸：传感器范围*2 / 栅格分辨率，强制为奇数（中心对齐机器人）
    ...
    img_mat_ = cv::Mat::zeros(MAT_SIZE, MAT_SIZE, CV_32FC1);        // 初始化占据图，空白占据图为0，障碍物点云对应像素值增加，最后二值化得到占据图
    img_counter_ = 0;
    odom_node_ptr_ = NULL;
...
// 主函数：构建占据图并提取轮廓 总入口函数，输入当前里程计节点和周围点云，输出提取的轮廓（世界坐标系）
void ContourDetector::BuildTerrainImgAndExtractContour(const NavNodePtr& odom_node_ptr,
    ...
    CVPointStack cv_corners;            // 角点对应的图像坐标，后续转换为世界坐标系
    PointStack corner_vec;              // 角点对应的世界坐标，后续转换为占据图坐标
    this->UpdateOdom(odom_node_ptr);    // 更新机器人当前位置
    this->ResetImgMat(img_mat_);        // 重置上一帧占据图
    this->UpdateImgMatWithCloud(surround_cloud, img_mat_);  // 核心：点云 转换 占据图
    this->ExtractContourFromImg(img_mat_, refined_contours_, realworl_contour); //提取 轮廓
}
...
// 第四步：提取 轮廓细化：使用Ramer–Douglas–Peucker算法，去除冗余点，保留关键点
